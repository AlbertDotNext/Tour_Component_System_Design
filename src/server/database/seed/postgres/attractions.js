...
writeAttractions.write(
  'id,name,latitude,longitude, description,rating,image_path,image_alt,created_at,updated_at\n',
  'utf8'
);
...
const writeOneMillion = (writer, encoding, callback) => {
  ...
  const write = () => {
    ...
    do {
      ...
      const latitude =
        Math.random() * (coords.north - coords.south) + coords.south;
      const longitude =
        Math.random() * (coords.east - coords.west) + coords.west;
      ...
      const updated_at = faker.date.future(10);
      const data = `${id},${name},${latitude},${longitude}, ${description},${rating},${image_path},${mage_alt},${created_at},${updated_at}\n`;
      if (i === 0) {
